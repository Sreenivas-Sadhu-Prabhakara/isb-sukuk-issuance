...
| POST | `/v1/sukuk-issuance-program-procedure/initiate` | Initiate |
| GET | `/v1/sukuk-issuance-program-procedure` | Retrieve (list) |
| GET | `/v1/sukuk-issuance-program-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/sukuk-issuance-program-procedure/{crId}/update` | Update |
| PUT | `/v1/sukuk-issuance-program-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/sukuk-issuance-program-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-sukuk-issuance:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-sukuk-issuance ./helm -n isb-treasury
```

Exposed through the platform Gateway at path prefix `/isb-sukuk-issuance` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
